  ...
  while (1) {
    ...
    if (result != UNZ_OK) {
      if (result != UNZ_END_OF_LIST_OF_FILE) {
        TRACE(@"MiniZip returned error %i", result);
        array = nil;
      }
      break;
    }
    ...
    if (path) {
      ...
      if ([path hasSuffix:@"/"]) {
        ...
        if (![manager createDirectoryAtPath:fullPath withIntermediateDirectories:YES attributes:nil error:&error]) {
          TRACE(@"Failed creating directory \"%@\" from ZIP archive: %@", path, error);
          success = NO;
        }
      }
      // Otherwise extract file
        ...
        if (outFile) {
          while (1) {
            ...
            if (read > 0) {
              if (fwrite(buffer, read, 1, outFile) != 1) {
                 TRACE(@"Failed writing \"%@\" from ZIP archive", path);
                 success = NO;
                 break;
              }
            } else if (read < 0) {
              TRACE(@"Failed reading \"%@\" from ZIP archive", path);
              success = NO;
              break;
            }
            else {
          ...
          TRACE(@"Failed creating file \"%@\" from ZIP archive (%s)", fullPath, strerror(errno));
          success = NO;
        }
      }
    }
  ...
  while (1) {
    ...
    if (![path hasSuffix:@"/"] && [path isEqualToString:inPath]) {
      ...
      if (outFile) {
        ...
        TRACE(@"Failed creating \"%@\" from ZIP archive", path);
      }
      unzCloseCurrentFile(_unzFile);
      break;
    }
